...
#   - Неинтерактивные флаги: --disable / --restore-last / --check / --status / --diag / --unlock
#   - Ротация лога, lock-файл против параллельного запуска с АВТО-снятием
#     осиротевшего экземпляра после обрыва SSH
#
# Изменения 2.1.4 -> 2.1.5:
#   - Меню больше не зацикливается на EOF (обрыв SSH/PTY): корректный выход, lock освобождается.
#   - Ловим HUP/INT/TERM -> чистый выход -> освобождение lock.
#   - acquire_lock авто-снимает осиротевший интерактивный экземпляр (ppid=1) —
#     больше не нужен ребут / ручное удаление /tmp/ipv6-off.lock.
#   - Ручной аварийный выход: --unlock.
#
...
############################################
# Осиротевший ли экземпляр держит lock?
# После обрыва SSH процесс теряет родителя и реапарентится на init (ppid=1) —
# это надёжный признак "зомби от оборванной сессии". Такой можно снимать.
# НЕ трогаем: рабочие процессы (--disable/--restore-last/--diag) и живые чужие
# интерактивные сессии (у них ppid = dropbear/ssh, не 1).
lock_is_orphan(){
  pid="$1"
  cl="$(tr '\0' ' ' < "/proc/$pid/cmdline" 2>/dev/null)"
  case "$cl" in *ipv6-off*) : ;; *) return 1 ;; esac          # точно наш скрипт (не переиспользованный PID)
  case "$cl" in *--disable*|*--restore-last*|*--diag*) return 1 ;; esac  # рабочий процесс — не трогаем
...
acquire_lock(){
  if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n "$oldpid" ] && kill -0 "$oldpid" 2>/dev/null; then
      if lock_is_orphan "$oldpid"; then
        log "acquire_lock: осиротевший экземпляр pid=$oldpid (ppid=1) — снимаю автоматически"
        kill "$oldpid" 2>/dev/null
        i=0
        while kill -0 "$oldpid" 2>/dev/null && [ "$i" -lt 3 ]; do sleep 1; i=$((i+1)); done
        kill -9 "$oldpid" 2>/dev/null || true
        rm -f "$LOCK_FILE" 2>/dev/null || true
      else
        ui_err "Уже запущен другой экземпляр (pid=$oldpid). Выход."
        ui_info "Если это зависший процесс после обрыва SSH: sh $0 --unlock"
        exit 1
      fi
    else
      rm -f "$LOCK_FILE" 2>/dev/null || true   # мёртвый/чужой PID — протухший lock, снимаем
    fi
  fi
...
# Взять lock и поставить ловушки так, чтобы ЛЮБОЙ выход (в т.ч. обрыв SSH -> HUP)
# гарантированно освобождал lock. Без этого осиротевший процесс держал lock до ребута.
arm_locking(){
  acquire_lock
...
Использование:
  ...
  ipv6-off.sh --unlock                     снять зависший lock (после обрыва SSH)
  ipv6-off.sh --help                       эта справка

Совет: если ожидаете обрыв SSH при отключении (например, активен podkop),
запускайте detached, чтобы обрыв сессии был безразличен:
  setsid sh $0 --disable --yes >/tmp/ipv6-off.run 2>&1 &
...
main(){
  ...
  case "${1:-}" in
    ...
    --unlock)
      op="$(cat "$LOCK_FILE" 2>/dev/null)"
      if [ -n "$op" ] && kill -0 "$op" 2>/dev/null; then
        kill "$op" 2>/dev/null
        i=0; while kill -0 "$op" 2>/dev/null && [ "$i" -lt 3 ]; do sleep 1; i=$((i+1)); done
        kill -9 "$op" 2>/dev/null || true
        echo "снят зависший экземпляр pid=$op"
      fi
      rm -f "$LOCK_FILE" 2>/dev/null || true
      echo "lock снят ($LOCK_FILE)"
      exit 0 ;;
    ...
    --diag)    arm_locking; collect_diag; exit 0 ;;
    --disable)
      arm_locking
      shift
      for a in "$@"; do
  ...
  while true; do
    menu
    # EOF на stdin (обрыв SSH/PTY) не должен зацикливать меню — выходим, lock снимется.
    if ! IFS= read -r opt; then
      log "menu: stdin EOF (обрыв SSH?) — выход, снимаю lock"
      exit 0
    fi
    case "$opt" in
